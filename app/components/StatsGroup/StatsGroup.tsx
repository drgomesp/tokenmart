...
const useStyles = createStyles((theme) => ({
    root: {
        display: 'flex',
        backgroundImage: `linear-gradient(-60deg, ${"silver"} 0%, ${
            "lightgray"
        } 100%)`,
...
export function StatsGroup({ data }: StatsGroupProps) {
    ...
    const stats = data.map((stat) => (
        <div key={stat.title} className={classes.stat}>
            <Text color={"dimmed"}
                  className={classes.count}>{stat.stats}</Text>
            <Text color={"dimmed"} className={classes.title}>{stat.title}</Text>
            <Text color={"dimmed"}
                  className={classes.description}>{stat.description}</Text>
        </div>
    ));
